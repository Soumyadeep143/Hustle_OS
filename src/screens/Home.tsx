...
// ── Time-of-day config ─────────────────────────────────────────────────
const TIME_CONFIG = {
  midnight: { word: 'midnight', cssClass: 'greeting-time--midnight' },
  dawn:     { word: 'dawn',     cssClass: 'greeting-time--dawn' },
  morning:  { word: 'morning',  cssClass: 'greeting-time--morning' },
  midday:   { word: 'midday',   cssClass: 'greeting-time--midday' },
  afternoon:{ word: 'afternoon',cssClass: 'greeting-time--afternoon' },
  dusk:     { word: 'dusk',     cssClass: 'greeting-time--dusk' },
  evening:  { word: 'evening',  cssClass: 'greeting-time--evening' },
  night:    { word: 'night',    cssClass: 'greeting-time--night' },
} as const;
...
function TypingText({ text, delay = 0, speed = 68, className = '', onDone }: {
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      if (indexRef.current >= text.length) {
        clearInterval(tick); setDone(true); onDoneRef.current?.();
      }
    }, speed);
...
function AnimatedSubtitle() {
  ...
  useEffect(() => {
    SUBTITLE_WORDS.forEach((_, i) => setTimeout(() => setVisible(i + 1), 1600 + i * 110));
  }, []);
  return (
    <p className="greeting-sub mt-1 text-[15px]">
      {SUBTITLE_WORDS.map((word, i) => (
        <span key={word} className={`greeting-sub-word${visible > i ? ' visible' : ''}`}>
...
const BRIEF_LABELS = ["TODAY'S BRIEF", 'आज की जानकारी', 'ಇಂದಿನ ಸಂಕ್ಷಿಪ್ತ', 'আজকের সংক্ষিপ্ত'];
function RotatingLabel({ words, intervalMs = 2600 }: { words: string[]; intervalMs?: number }) {
  ...
  return <span key={i} className="ai-brief-eyebrow-lang">{words[i]}</span>;
}
...
export function Home() {
  ...
  return (
    <div className="flex flex-col gap-6 px-5 pt-6 md:px-0 md:pt-8">
  ...
  return (
    <>
      ...
      <div className="greeting-block">
        <h1 className="greeting-heading font-[var(--font-display)] text-[30px] font-semibold leading-[1.1] tracking-[-.028em] text-[var(--color-ink)] md:text-[36px]">
          {memory ? (
            <>
              <TypingText text="Good " delay={0} speed={72} className="greeting-word greeting-good" />
              <TypingText text={timeConf.word} delay={timeWordDelay} speed={72} className={`greeting-word greeting-time ${timeConf.cssClass}`} />
              <span className="greeting-comma">,{'\u00a0'}</span>
              <TypingText text={memory.user_profile.name.split(' ')[0]} delay={nameDelay} speed={68} className="greeting-word greeting-name" />
      ...
      {/* ── md+: two-column grid (brief + today LEFT, focus + signals RIGHT) ── */}
      <div className="grid grid-cols-1 gap-6 md:grid-cols-[1fr_1fr] lg:grid-cols-[1.1fr_0.9fr]">

        {/* LEFT column */}
        <div className="flex flex-col gap-6">
          {/* AI Brief */}
          <div className="ai-brief-card rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-surface)] p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="font-[var(--font-mono)] text-[10.5px] text-[var(--color-ink-3)]">
                <RotatingLabel words={BRIEF_LABELS} />
              </span>
            </div>
            <p key={brief?.headline ?? 'loading'} className={`ai-brief-headline mt-3 font-[var(--font-display)] font-semibold text-[var(--color-ink)]${brief ? '' : ' is-loading'}`}>
              {brief?.headline ?? 'Loading…'}
            </p>
            <div className="mt-3 flex items-center justify-between border-t border-[var(--color-line-2)] pt-3">
              <button onClick={() => navigate('/ai')} className="text-[13px] font-semibold text-[var(--color-blue)]">View plan →</button>
              <button onClick={() => navigate('/ai')} className="text-[13px] text-[var(--color-ink-2)]">Ask</button>
            </div>
          </div>
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setTimelineSheetEntry(null)} className="text-[12.5px] font-medium text-[var(--color-blue)]">+ Add</button>
            </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setSignalSheetItem(null)} className="text-[12.5px] font-medium text-[var(--color-blue)]">+ Add</button>
            </div>
            <div className="flex flex-col gap-2">
              {signals.map((s) => (
                <button key={s.id} onClick={() => setSignalSheetItem(s)}
      ...
      <EditTimelineEntrySheet
        open={timelineSheetEntry !== undefined} onClose={() => setTimelineSheetEntry(undefined)}
      ...
      />
      <EditSignalSheet
        open={signalSheetItem !== undefined} onClose={() => setSignalSheetItem(undefined)}
...
function HomeTeam() {
  ...
  useEffect(() => { api.team.getState('default').then(setState); }, []);

  if (!state) return <div className="h-40 animate-pulse rounded-[var(--radius-card)] bg-[var(--color-line-2)]" />;

  const totalCapacity   = state.members.reduce((s, m) => s + m.capacity_hours, 0);
  const totalAssigned   = state.members.reduce((s, m) => s + m.assigned_hours, 0);
  ...
  return (
    <>
      ...
      {state.current_recommendation && (
        <div className="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-surface)] p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
      ...
      <button onClick={() => navigate('/team')} className="flex items-center justify-center gap-1.5 rounded-[var(--radius-control)] border border-[var(--color-line)] py-3 text-[13.5px] font-semibold text-[var(--color-blue)]">
        View full team →
      </button>
    </>
  );
...
function HomeEnterprise() {
  ...
  useEffect(() => { api.workspace.getOrgHealth().then(setOrg); }, []);

  if (!org) return <div className="h-40 animate-pulse rounded-[var(--radius-card)] bg-[var(--color-line-2)]" />;
    ...
    <>
      <div>
        ...
        <div className="mt-3 flex items-baseline gap-3">
          <span className="font-[var(--font-display)] text-[68px] font-semibold leading-none tracking-[-.05em] text-[var(--color-ink)]">{org.execution_health}%</span>
      ...
      <div className="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-x-8">
        {org.rows.map((r) => (
          <div key={r.label} className="flex items-center justify-between border-b border-[var(--color-line)] py-3.5 last:border-b-0 md:last:border-b">
            <span className="text-[15px] text-[var(--color-ink-2)]">{r.label}</span>
            <span className="font-[var(--font-display)] text-[24px] font-semibold" style={{ color: r.tone ? `var(--color-${r.tone})` : 'var(--color-ink)' }}>{r.value}</span>
          </div>
        ))}
      </div>
      <div className="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-surface)] p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
        ...
        <div className="mt-2 flex flex-col gap-1.5">
          {org.insight_lines.map((l, i) => (
            <p key={i} className="text-[14.5px] leading-relaxed" style={{ color: l.tone ? `var(--color-${l.tone})` : 'var(--color-ink)' }}>{l.text}</p>
          ))}
        </div>
      </div>
    </>
